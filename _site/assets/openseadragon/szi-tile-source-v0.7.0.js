class h {
  ...
  static fetchFileSize = async (t, e) => {
    const n = e.headers ? { ...e.headers, Range: "bytes=0-255" } : { Range: "bytes=0-255" }, i = await fetch(t, {
      headers: n,
      mode: e.mode,
      credentials: e.credentials
    });
    if (!i.ok)
      throw new Error(`Could not fetch size of ${t}, response status for request: ${i.status}`);
    const r = i.headers.get("Content-Range");
    if (!r)
      throw new Error(
        `Could not fetch size of ${t}, Content-Range header not included in response. Check that your server's CORS settings include it in Access-Control-Expose-Headers.`
      );
    const [a, s, c, d] = r.match(/bytes (\d+)\-(\d+)\/(\d+)/);
    if (!a || !d)
      throw new Error(`Could not fetch size of ${t}, Content-Range header didn't contain the length of the file`);
  ...
  fetchRange = async (t, e, n) => {
    ...
    if (!a.ok)
      throw new Error(`Couldn't fetch range ${t}:${e} of ${this.url} status: ${a.status}`);
...
const l = 4294967295, m = 65535, S = m, z = 22, U = 20, O = 1, b = 101010256, D = 117853008, R = 101075792, C = 33639248, L = 67324752;
function $(o, t, e) {
...
function x(o, t, e) {
  ...
  for (; o.pos - s < t; ) {
    ...
    c === O ? (i === l && (i = o.readUint64()), n === l && (n = o.readUint64()), a === l && (a = o.readUint64()), r === m && (r = o.readUint32())) : o.skip(d);
  }
  return {
    compressedSize: n,
    uncompressedSize: i,
...
function N(o, t) {
  ...
  for (let i = 0; i < t; i++) {
    ...
    const a = e.readUint32(), s = e.readUint32(), c = e.readUint16(), d = e.readUint16(), f = e.readUint16(), y = e.readUint16();
    e.readUint16(), e.readUint32();
    const E = e.readUint32(), g = e.readUtf8String(c);
    if (a !== s)
      throw new Error(
    ...
    const p = x(e, d, {
      ...
      uncompressedSize: s,
      diskNumberStart: y,
      relativeOffsetOfLocalHeader: E
    });
    e.readUtf8String(f), n.push({
      uncompressedSize: p.uncompressedSize,
      relativeOffsetOfLocalHeader: p.relativeOffsetOfLocalHeader,
      filename: g
    });
  }
  return n;
...
async function B(o) {
  const t = Math.max(0, o.size - (U + z + S)), e = await o.fetchRange(t, o.size), { totalEntries: n, centralDirectoryOffset: i, centralDirectorySize: r, startOfEocdInBuffer: a } = I(e);
  if (n === m || i === l || r === l) {
    const c = a - U, d = k(e, c), f = await o.fetchRange(
      d.zip64EocdOffset,
      t + c
    );
    return v(f, 0);
...
class w {
   ...
   * @returns {string}
   */
  tilesDirectory = () => this.dziFilename().replace(/\.dzi$/, "_files/");
}
const Z = (o) => {
  class t extends o.DziTileSource {
    ...
    downloadTileStart = (n) => {
      const i = new Image();
      i.onload = function() {
        r(), n.finish(i, n.userData.request, "image");
